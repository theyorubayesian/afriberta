...
from ner_scripts.ner_utils import convert_examples_to_features
from ner_scripts.ner_utils import get_labels
from ner_scripts.ner_utils import read_examples_from_file
...
def main():
    ...
    parser.add_argument(
        "--input_dir", default=None, type=str, required=False, help="The input model directory.",
    )
    parser.add_argument(
        "--output_dir",
        default=None,
    ...
    model = XLMRobertaForTokenClassification.from_pretrained(args.model_name_or_path, config=config)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model = XLMRobertaForTokenClassification.from_pretrained(checkpoint)
            model.to(args.device)
            result, _ = evaluate(
                args, model, tokenizer, labels, pad_token_label_id, mode="dev", prefix=global_step
            )
            if global_step:
